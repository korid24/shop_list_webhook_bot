from config import REPLACE_SEPARATOR

WELCOME_TEXT = '''Приветствую! С моей помощью вы можете создавать списки \
покупок и добавлять в них свои покупки. Для того чтобы создать список просто \
напишите мне его название. Для того чтобы войти в список напишите его порядковый \
номер, для того чтобы создать покупку внутри списка напишите название \
покупки. Для удаления элемента(списка или покупки) напишите \'-\', а \
затем порядковый номер элемента (например -1). Для возвращение в перечень \
списков наберите вверх. Для вызова справки наберите /help\n\n'''

HELP_TEXT = '''Находясь в меню списков можно выполнить следующие дейсвия:
